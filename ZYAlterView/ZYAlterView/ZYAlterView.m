...
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message sureBtn:(NSString *)sureTitle cancleBtn:(NSString *)cancleTitle
{
    if (self == [super init]) {
        ...
        self.backgroundColor = [UIColor colorWithDisplayP3Red:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.5];
        ...
        if (title) {
            ...
            self.titleLbl = [self GetAdaptiveLable:CGRectMake(2*ZYSpace, 2*ZYSpace, AlertW-4*ZYSpace, 20) AndText:title andIsTitle:YES];
            ...
            self.titleLbl.frame = CGRectMake((AlertW-titleW)/2, 2*ZYSpace, titleW, titleH);
            
        }
        if (message) {
            
            self.msgLbl = [self GetAdaptiveLable:CGRectMake(ZYSpace, CGRectGetMaxY(self.titleLbl.frame)+ZYSpace, AlertW-2*ZYSpace, 20) AndText:message andIsTitle:NO];
            ...
            self.msgLbl.frame = self.titleLbl?CGRectMake((AlertW-msgW)/2, CGRectGetMaxY(self.titleLbl.frame)+ZYSpace, msgW, msgH):CGRectMake((AlertW-msgW)/2, 2*ZYSpace, msgW, msgH);
        }
        
        self.lineView = [[UIView alloc] init];
        self.lineView.frame = self.msgLbl?CGRectMake(0, CGRectGetMaxY(self.msgLbl.frame)+2*ZYSpace, AlertW, 1):CGRectMake(0, CGRectGetMaxY(self.titleLbl.frame)+2*ZYSpace, AlertW, 1);
